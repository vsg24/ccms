 ...
 * @copyright  2015 Vahid Amiri Motlagh
 * @license    http://opensource.org/licenses/MIT MIT License
 * @link       http://github.com/vsg24/sitemap-php
 */
class Sitemap {
	...
	private $writer;
	private $domain;
	private $path; // MUST END WITH A FORWARD SLASH
	private $filename = 'sitemap';
	private $current_item = 0;
	private $current_sitemap = 0;
	private $xml_maps_count = 0;
	...
	private function startSitemap() {
		...
		if ($this->getCurrentSitemap()) {
			$this->getWriter()->openURI($this->getPath() . $this->getFilename() . self::SEPERATOR . $this->getCurrentSitemap() . self::SEPERATOR . self::SITEMAP_SUFFIX . self::EXT);
		} else {
			$this->getWriter()->openURI($this->getPath() . $this->getFilename() . self::SEPERATOR . self::SITEMAP_SUFFIX . self::EXT);
		}
	 ...
	 *
	 * @return array file names without extension
	 */
	public function getAllSitemaps()
	{
		$files = [];
		foreach (glob($this->path . "/*-sitemap.xml") as $filename) {
			$this->xml_maps_count += 1;
	...
	public function createSitemapIndex($loc, $lastmod = 'Today')
	{
		...
		$indexwriter->openURI($this->getPath() . self::SITEMAP_SUFFIX . self::SEPERATOR . self::INDEX_SUFFIX . self::EXT);
		$indexwriter->startDocument('1.0', 'UTF-8');
		$indexwriter->setIndent(true);
		$indexwriter->startElement('sitemapindex');
		$xmlmaps_names = $this->getAllSitemaps();
		$indexwriter->writeAttribute('xmlns', self::SCHEMA);
		for ($index = 0; $index < $this->xml_maps_count; $index++) {
			$indexwriter->startElement('sitemap');
			$indexwriter->writeElement('loc', $loc . $xmlmaps_names[$index] . self::EXT);
